...
citiesArray.forEach(element => {
  ...
  fetch(getIndicatorsDataBaseURL, element.getIndicatorsDataForm)
    ...
    .then(json => {
      ...
      return fetch(AQIhourlyDataBaseURL, element.hourlyAQIform);
    })
    .then(response => response.json())
    .then(json => {
      let data = { ...json };
      ...
      let at_time_replaced2 = atTime.replace(/\s/g, "_");
      let at_time_replaced3 = at_time_replaced2.replace(/:/g, "h");
      fs.writeFileSync(
        `data/CEM/${element.city}/Hourly-AQI-Data/${at_time_replaced3}.json`,
